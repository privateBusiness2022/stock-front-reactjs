...
// sections
import { CommissionsTableToolbar, CommissionsTableRow } from '../../sections/@dashboard/user/commissionsList';
import useLocales from '../../hooks/useLocales';
import { getById, getAll, getCommissions } from '../../redux/slices/dividing';
...
export default function StageCommissionProfile() {
  const {
    dense,
    page,
    order,
  ...
  const { commissions } = useSelector((state) => state.stages);

  const { enqueueSnackbar } = useSnackbar();

  const [tableData, setTableData] = useState(commissions);

  if (tableData === undefined) {
    window.location.replace(PATH_DASHBOARD.dividing.commissions);
  }
  ...
  const STATUS_OPTIONS = [{ title: translate('dividing-list.all-commissions'), value: 'ALL' }];
  ...
  const TABLE_HEAD = [
    ...
    { id: 'number', label: translate('investors-list.commission'), align: 'left' },
  ...
  return (
    <Page title={translate('dividing-list.commission-holder')}>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          heading={translate('dividing-list.commission-holder')}
          links={[
            { name: translate('Sidebar.dashboard'), href: PATH_DASHBOARD.root },
            { name: translate('Sidebar.users'), href: PATH_DASHBOARD.investor.root },
            { name: translate('Sidebar.commission-holder-list') },
          ]}
        />
        ...
        <Card>
          ...
          <CommissionsTableToolbar filterName={filterName} onFilterName={handleFilterName} />
