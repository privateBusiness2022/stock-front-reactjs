...
export default function CommissionList({ show = true }) {
  const {
    dense,
    page,
    order,
  ...
  return (
    <Page title={translate('dividing-list.commissions-list')}>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        {show ? (
          <HeaderBreadcrumbs
            heading={translate('dividing-list.commissions-list')}
            links={[
              { name: translate('Sidebar.dashboard'), href: PATH_DASHBOARD.root },
              { name: translate('Sidebar.dividing'), href: PATH_DASHBOARD.period.root },
              { name: translate('Sidebar.commissions-list') },
            ]}
          />
        ) : null}
                ...
                <TableBody>
                  {dataFiltered.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
                    <DividingTableRow
                      ...
                      onClickRow={() => navigate(PATH_DASHBOARD.dividing.profile(paramCase(`${row.id}`)))}
                    />
                  ))}
